...
const PLAYER_ORIGIN = 'https://superflixapi.pro';
...
function findToken(html: string): string | null {
  const patterns = [
...
async function postForm(url: string, body: URLSearchParams, extra: Record<string, string> = {}) {
  return fetch(url, {
    method: 'POST',
    headers: {
...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    const playerPath = isSeries
      ? `/serie/${id}/${Number(season)}/${Number(episode)}`
      : `/filme/${id}`;
    const playerUrl = `${PLAYER_ORIGIN}${playerPath}`;

    // 1) Load the Player 1 page and obtain its short-lived page token.
    const page = await fetch(playerUrl, {
      headers: { 'User-Agent': UA, Accept: 'text/html,application/xhtml+xml', Referer: PLAYER_ORIGIN + '/' },
    });
    ...
    // 2) Same /player/bootstrap request used by Player 1.
    ...
    const server = options.find((item: any) => item?.ID && item?.is_file === false) || options.find((item: any) => item?.ID);
    if (!server?.ID) return res.status(502).json({ error: 'No Player 1 server returned by bootstrap' });

    // 3) Same /player/source request used by Player 1.
    const sourceBody = new URLSearchParams({
      video_id: String(server.ID),
      page_token: pageToken,
      host: '',
      site: '',
      _token: '',
    ...
    // 4) Follow the redirect until the /video/<hash> page is reached.
    const redirect = await fetch(redirectUrl, {
      redirect: 'follow',
      headers: { 'User-Agent': UA, Accept: 'text/html,application/xhtml+xml', Referer: playerUrl },
    });
    ...
    // 5) Exact getVideo request from the Player 1 network capture.
    const getVideoBody = new URLSearchParams({
      hash,
      r: `${PLAYER_ORIGIN}/`,
    });
    ...
    if (typeof securedLink !== 'string' || !/^https?:\/\//i.test(securedLink)) {
      return res.status(502).json({ error: 'securedLink was not returned' });
    }

    return res.status(200).json({
      streamUrl: securedLink,
      referer: finalParsed.origin,
      kind: 'hls',
      source: 'securedLink',
    });
  } catch (error) {
